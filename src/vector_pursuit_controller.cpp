  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  declare_parameter_if_not_declared(
    node, plugin_name_ + ".k_yaw", rclcpp::ParameterValue(3.0));
  declare_parameter_if_not_declared(
  ...
  const double & pose_cost, double & linear_vel, const nav_msgs::msg::Path & path, double & sign)
{
    ...
    pose_cost != static_cast<double>(nav2_costmap_2d::FREE_SPACE))
  {
    ...
    RCLCPP_INFO(logger_,"min_distance_to_obstacle: %f",min_distance_to_obstacle);
  ...
  std::vector<rclcpp::Parameter> parameters)
{
  ...
  for (auto parameter : parameters) {
    ...
    if (type == ParameterType::PARAMETER_DOUBLE) {
      ...
      } else if (name == plugin_name_ + ".k_yaw") {
        k_yaw_ = parameter.as_double();
